...
\begin{remark}[Asymmetric correction in the non-balanced CFBM]\label{rem:C-asym}
...
Under \Cref{def:cfbm}, $\vv$ is constant on each clan (\Cref{lem:A-decomp}), so
...
Under \Cref{def:cfbm} and the observation model \eqref{eq:sampling_estimator}, with
$\bze=\Sout^{\max}$ and using
$(c_A+c_B)^2=m/(|A||B|)=(1+\eta)^2/(m\eta)$,
\begin{equation}
...
$|\vv_i-\vv_j|=c_A+c_B$, so $\sigma_B^2=\tfrac{1-p}{p}(c_A+c_B)^2|A|\bze^2$;
...
Under \Cref{lem:A-decomp}, $\min_i|\vv_i|=|c_B|$, attained on the larger clan $B$. The
