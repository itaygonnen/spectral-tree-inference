...
Under \Cref{asm:cfbm}, $\vv$ is constant on each clan (\Cref{lem:A-decomp}), so
...
Under \Cref{asm:cfbm} and the observation model \eqref{eq:sampling_estimator}, with
...
Under \Cref{asm:cfbm}, $\min_i|\vv_i|=|c_B|$, attained on the larger clan $B$. The
