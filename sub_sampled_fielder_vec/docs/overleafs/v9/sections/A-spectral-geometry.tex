...
We first record the range of the population entries, which earlier versions carried
as a standing assumption and which is in fact a consequence of the three that
precede it.

\begin{remark}[Range of the population entries]\label{rem:range}
Under \Cref{asm:gtr,asm:clock,asm:cfbm} the population matrix satisfies
\[
   S_{ii}=1,\qquad
   0<\bze\le S_{ij}\le\Smax<1\quad(i\ne j),
   \qquad \Smax:=\max_{i\ne j}S_{ij}.
\]
Indeed $\tau_{ii}=0$ makes $S_{ii}=\exp(0)=1$ --- consistent with
\eqref{eq:sampling_estimator}, whose unbiasedness $\E[\hat S]=S$ requires it ---
while off the diagonal \Cref{asm:clock} places every cross-clan entry at
$\bze\in(0,1)$ and \eqref{eq:cfbm_floor} places every same-clan entry in
$(\bze,\Smax]$. No range claim is made for $\hat S$: by
\eqref{eq:sampling_estimator} its observed off-diagonal entries equal $S_{ij}/p$,
which exceed $1$ throughout the regime $p=\Theta(\log m/m)$ of interest.
\end{remark}

\noindent Every later appeal to a bounded entry is to this remark. For the proof of
\Cref{lem:gap} we next derive the spectral properties of the
Fiedler vector.

\begin{lemma}[Spectral decomposition of the imbalanced Laplacian]\label{lem:A-decomp}
Under \Cref{asm:clock,asm:cfbm}: (i) the smallest eigenvalue is $\lambda_1=0$ with
...
\Cref{asm:cfbm} already carry the row sums of their own block,
...
The argument uses \Cref{asm:clock,asm:cfbm} and \Cref{rem:range}. By \Cref{lem:A-decomp} the
