...
scoring each sub-sampled split against the one the full matrix returns
\cite{strehlghosh2002cluster, fortunato2010community} and stratifying by imbalance
rather than pooling.
\end{itemize}

\Cref{sec:model} fixes the generative model and states the recovery result that
makes the similarity matrix the object of interest; \Cref{sec:problem} gives the
cost that motivates sampling, the observation model and the two operators we
analyse. \Cref{sec:main} states the Laplacian rate and \Cref{app:dist} the
distance rate. \Cref{sec:bridging} then routes tree shape into the spectrum through
$\eta$: it identifies which split the population Fiedler vector returns and bounds
the two failure modes. \Cref{sec:outline} assembles the proof and
\Cref{sec:empirical} tests the rates.
