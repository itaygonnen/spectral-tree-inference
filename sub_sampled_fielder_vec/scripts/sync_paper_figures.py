...
def write_map() -> None:
    ...
    A("")
    A(f"All {len(PF.SUPPORTING)} are kept. This table exists so nobody mistakes them for figure sources,")
    A("and so the evidence behind the distance route stays findable. `thm:main-dist`")
    A("does carry figures — Figs 4 and 5 of §6 — but they are produced from")
    A("`supporting/`, not from `paper/`, so this table is where their siblings live.")
    A("")
