#!/usr/bin/env python3
"""Non-interactive runner for the operator screen and the recovery sweep.

Two stages, both resumable, both driven by :mod:`src.runners.experiment_run`:

    screen   eta + validity per tree for every operator  -> one .npz of rows per dataset
    sweep    NMI vs sub-sampling rate p for the same     -> one .npz per tree
...
"real data") asks the same questions in a menu and then calls the same ``execute``, so
the two produce identical run directories.

    python scripts/run_sweep.py --list
    python scripts/run_sweep.py --dataset "1000 taxa,6000 taxa" --stage screen --workers 8
    nohup python scripts/run_sweep.py --dataset "6000 taxa" --stage sweep \
        > logs/real_sweep.log 2>&1 &

The sweep defaults to the trees whose partition is a real tree edge under BOTH L(S) and
B, and whose reference split is no more lopsided than eta=20; ``--dataset-rule`` widens
the first gate, ``--max-eta 0`` removes the second.
"""
...
from analysis.utils.real_datasets import (describe_search, get_dataset,   # noqa: E402
                                          list_datasets)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--dataset-rule", "--cohort-rule", "--gate", dest="dataset_rule",
...
def spec_from_args(args) -> RunSpec:
    ...
    return RunSpec(stage=args.stage, max_trees=args.limit, workers=args.workers,
                   gate=args.dataset_rule, max_eta=args.max_eta, p_min=args.p_min,
                   p_points=args.p_points, reps=args.reps, num_gaps=args.num_gaps,
...
def sources_from_args(args):
    """The datasets named on the command line, as run sources."""
